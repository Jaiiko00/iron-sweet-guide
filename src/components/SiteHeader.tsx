    ...
    <header className="sticky top-0 z-40 animate-fade-in border-b border-border/70 bg-background/70 backdrop-blur-xl">
      <div className="mx-auto flex max-w-6xl flex-wrap items-center gap-x-6 gap-y-3 px-5 py-3">
        <Link to="/" className="group flex items-center gap-2">
          <HeartLogo className="h-9 w-9 transition-transform duration-300 group-hover:scale-110" />
        ...
        <nav className="flex flex-1 flex-wrap items-center gap-1 text-sm">
          {links.map((l) => (
            <Link
              ...
              className="rounded-full px-3 py-1.5 text-muted-foreground transition-all duration-300 hover:-translate-y-0.5 hover:bg-secondary hover:text-foreground"
        ...
        <div className="flex items-center gap-3 text-sm">
          ...
          <Button variant="outline" size="sm" className="transition-transform hover:scale-105" onClick={logout}>
            Salir
          </Button>
        </div>
      </div>
